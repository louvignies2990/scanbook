...
Contour contour;
void setup() {
  src = loadImage("test8.jpg");
  //src.resize(700,749);
  size(src.width + cardWidth, src.height);
  opencv = new OpenCV(this, src);
  opencv.blur(1);
  opencv.threshold(120);                                                              //120 correspons to the color to separate shapes
  contour = opencv.findContours(false, true).get(0).getPolygonApproximation();
  card = createImage(cardWidth, cardHeight, ARGB);                                    //create a copy of an image
  opencv.toPImage(warpPerspective(contour.getPoints(), cardWidth, cardHeight), card);//convertisseur d'image-
}
Mat getPerspectiveTransformation(ArrayList<PVector> inputPoints, int w, int h) {
  println(inputPoints+"yolo");
